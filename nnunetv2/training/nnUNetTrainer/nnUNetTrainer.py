...
from nnunetv2.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss, DC_and_topk_loss, DC_and_Focal_loss, DC_and_MultiClassFocal_loss, DC_and_CE_and_Focal_loss
...
class nnUNetTrainer(object):
    ...
    def _build_loss(self):
        # Parameters for different loss components
        dice_kwargs = {'batch_dice': self.configuration_manager.batch_dice,
                    'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}
        ce_kwargs = {}
        focal_kwargs = {'alpha': 0.25, 'gamma': 2.0}

        # Check if regions are present and choose appropriate loss
        if self.label_manager.has_regions:
            ...
            print("Loss: DC_and_CE_and_Focal_loss")
            loss = DC_and_CE_and_Focal_loss(dice_kwargs, ce_kwargs, focal_kwargs,
                                            weight_ce=1, weight_dice=1, weight_focal=1,
                                            ignore_label=self.label_manager.ignore_label,
                                            dice_class=MemoryEfficientSoftDiceLoss)

        # Compile the dice component if applicable
        if self._do_i_compile():
